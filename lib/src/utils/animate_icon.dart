...
class _AnimateIconsState extends State<AnimateIcons>
  ...
  Widget build(BuildContext context) {
    ...
    Widget first() {
      final icon = Icon(widget.startIcon, size: widget.size, color: Colors.black,);
      return Transform.rotate(
        angle: widget.clockwise ?? false ? angleX : -angleX,
        child: Opacity(
          opacity: y,
          child: GestureDetector(child: icon, onTap: ()=>_onStartIconPress(),)
        ),
      );
    }

    Widget second() {
      final icon = Icon(widget.endIcon, color: Colors.black,size: widget.size);
      return Transform.rotate(
        angle: widget.clockwise ?? false ? -angleY : angleY,
        child: Opacity(
          opacity: x,
          child: GestureDetector(child: icon, onTap: ()=>_onEndIconPress(),),
        ),
      );
    }
